...
**Why:** This is the optimal O(n) solution that demonstrates understanding of sliding window technique and frequency counting.

**Interview Logic:**
1. "I'll use a sliding window of size len(p) over string s"
2. "I'll maintain character frequency counts for the window and p"
...
- **Sorting:** "We could sort each substring and compare, but that's O(n*m log m)"
...
### 1. Brute Force
Check every possible substring of length p.length() for anagram.
