/**
 * Brute Force Approach - Find All Anagrams in a String
 * 
 * Logic:
 ...
 * 
 * Time Complexity: O(n*m*log m) - n substrings, each sorted (m = p.length)
 * Space Complexity: O(m) - for sorting and substring extraction
 */
class Solution {
...
- Total: O((n-m+1) * m * log m) ≈ O(n*m*log m)
- For n=10^4, m=10^4, this is very slow

Key limitation: No optimization to reuse previous computations
*/
